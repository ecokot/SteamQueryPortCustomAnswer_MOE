...
class QueryServer:
    ...
    async def main(self):
        ...
        for log_file in self.log_files:
            asyncio.create_task(parse_log(log_file, self.players, self.player_log_files))
    ...
    async def route_request(self, data, addr):
        ...
        if info_query_pattern.match(data):
            return await handle_info_query(data, addr, self.players)
        ...
        if player_query_pattern.match(data):
            return await handle_player_query(data, addr, self.challenge_numbers, self.players)
